...
describe('Calculator Strategy', () => {
  ...
  beforeEach(() => {
    ...
    calculatorStrategy = new CalculatorStrategy(
      calculator,
      randomStringService,
    );
  });
  ...
  describe('Math operations', () => {
    ...
    `(
      'must calculate operations',
      ({ operation, input, result, expectedFunction }: Props) => {
        jest.spyOn(calculator, expectedFunction).mockReturnValueOnce(result);
  ...
  describe('Random String', () => {
    it('must return a random string', async () => {
      ...
      const response = await calculatorStrategy.calculate(
  ...
  describe('Unknown operation', () => {
    it('must throw an error', () => {
      ...
      expect(calculate).toThrowError(
        `Operation unknown-operation not implemented`,
      );
